# Read image and annotations from LIDC.dicom with pylidc
# Save under CounterAlign_output/preprcess/raw
...
class LIDCData():
    ...
    def find_threeDloc(self, cmask, cbbox):
        ...
        for ind in range(cbbox[2].stop - cbbox[2].start):
            ...
            if len(contours) == 0:
                ...
                for contour in contours:
                    xmin = np.min(contour[:, 0]) * self.real_resize_factor[0]; xmax = np.max(contour[:, 0]) * self.real_resize_factor[0]
                    ymin = np.min(contour[:, 1]) * self.real_resize_factor[1]; ymax = np.max(contour[:, 1]) * self.real_resize_factor[1]
        ...
        startSlice = threeDloc[0][0]; stopSlice = threeDloc[-1][0]+1
        ...
        while sliceind <= stopSlice:
            ...
            for loc in threeDloc:
                if loc[0] == sliceind:
                    out.append(loc); isfind = True; break
            # pad previous slice
            if not isfind:
    ...
    def patch_nod(self, verbose=False):
        ...
        """
        outputs = {'nodimg': [], 'nodslice': [], 'nodbox': [], 'nodloc': [], 'nodmalig': [], 'nodsub': [], 'nodinterstr': [], 'nodcal': [],
                   'nodspher': [], 'nodmargin': [],
        ...
        for idx, nodule in enumerate(self.ann):
            ...
            # the nodule is not exactly centered in the consensused image, so, we need to find its box from mask
            ind = int(0.5 * (cbbox[2].stop - cbbox[2].start))
            contours = find_contours(cmask[:, :, ind].astype(float), 0.5)
            if (len(contours) == 0):
                continue
            nodbboxs = list()
            ...
            nodbboxs = np.array(nodbboxs)
            # there maybe multiple contours detected, we choose the maximum covering box
            nodbbox = [nodbboxs[:, 0].min(), nodbboxs[:, 1].min(), nodbboxs[:, 2].max(), nodbboxs[:, 3].max()]
            ...
            centerInd = int(0.5 * (rbbox[2].stop - rbbox[2].start)); nodslice = int(0.5 * (rbbox[2].stop + rbbox[2].start))
            nodvol = self.vol[rbbox][:, :, centerInd]

            # three-D location of the nodule in the re-sampled image, list of tuple
            threeDloc = self.find_threeDloc(cmask, cbbox)

            outputs['nodimg'].append(nodvol); outputs['nodslice'].append(nodslice); outputs['nodbox'].append(nodbbox)
            outputs['nodloc'].append(threeDloc)
            outputs['nodmalig'].append(malignancy); outputs['nodsub'].append(subtlety); outputs['nodinterstr'].append(internalStructure)
            outputs['nodcal'].append(calcification); outputs['nodspher'].append(sphericity); outputs['nodmargin'].append(margin)
            outputs['nodlob'].append(lobulation); outputs['nodspi'].append(spiculation); outputs['nodtext'].append(texture)

            if verbose:
                nodvol = np.stack([nodvol, nodvol, nodvol], -1)
                cv2.rectangle(nodvol, (nodbbox[1], nodbbox[0]), (nodbbox[3], nodbbox[2]), (0, 0, 255), 1)
                cv2.imwrite('{}-{}.png'.format(self.pid, idx), nodvol * 255)
...
def data_process(pid,PAD_XY):
    ...
    '''
    outputs = {'filename': [], 'slice': [],'box': [], 'loc': [], 'malignancy': [], 'subtlety': [], 'internalStructure': [],
    ...
    for ind, nodimg in enumerate(output['nodimg']):
        filename = pid + '-{}'.format(ind)
        np.save(os.path.join(BASE, filename), nodimg)
        outputs['filename'].append(filename)

    # save annotations
    outputs['slice'] += output['nodslice']; outputs['box'] += output['nodbox']
    outputs['loc'] += output['nodloc']
    outputs['malignancy'] += output['nodmalig']; outputs['subtlety'] += output['nodsub']
    outputs['internalStructure'] += output['nodinterstr']; outputs['calcification'] += output['nodcal']
    outputs['sphericity'] += output['nodspher']; outputs['margin'] += output['nodmargin']
    outputs['lobulation'] += output['nodlob']; outputs['spiculation'] += output['nodspi']; outputs['texture'] += output['nodtext']
...
if __name__ == '__main__':
   ...
   pids = os.listdir('/data/liumingzhou')
   BASE = '/home/liumingzhou/CounterAlign_output/preprocess/single_slice/raw/'.format(args.padsize)
   os.makedirs(BASE,exist_ok=True)

   processor = partial(data_process, PAD_XY=args.padsize)

   p = multiprocessing.Pool(16)
   logs = list(p.map(processor, pids))
   p.close()
   p.join()
