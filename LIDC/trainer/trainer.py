...
from Counter_align.LIDC.implicit_gradient.attributes2label import get_implicit_a2lmodel
from Counter_align.LIDC.implicit_gradient.image2attributes import get_implicit_i2amodel
...
class WholeTrainer:  # whole hierachical trainer
    ...
    def __init__(self, device, config):
        ...
        self.public_tensors = {'train_i2a_attention_mask': [mask_gt for mask_gt, _, _, _, _ in self.train_data_loader],
                               ...
                               'test_i2a_attention_mask': [mask_gt for mask_gt, _, _, _, _ in self.test_data_loader],
                               ...
                               'test_a2l_attention_mask': [ccce_gt for _, _, ccce_gt, _, _ in self.test_data_loader],

                               'train_i2a_att_feat': [], 'val_i2a_att_feat': [], 'test_i2a_att_feat': [],
                               'train_modified_att': [], 'val_modified_att': [], 'test_modified_att': [],
                               ...
                               'test_pred_ccce_gt': [ccce_gt for _, _, ccce_gt, _, _ in self.test_data_loader],
    ...
    def train(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            # base="/data/lijingwen/CounterAlign_output/hierachical_model/DDSM/240702_193524_124_1/models"
            # self.i2a_clser.load_state_dict(
            #     torch.load(os.path.join(base, f'i2a_clser/i2a_cls_epoch:{epoch}_best.pt'), map_location=self.device))
            self.i2a_clser_trainer.train(epoch)

            # self.a2l_clser.load_state_dict(
            #     torch.load(os.path.join(base, f'a2l_clser/a2l_cls_epoch:{epoch}_best.pt'), map_location=self.device))
            self.a2l_clser_trainer.train(epoch)#att_pred

            # self.a2l_atter.load_state_dict(
            #     torch.load(os.path.join(base, f'a2l_atter/a2l_att_epoch:{epoch}_best.pt'), map_location=self.device))
            self.a2l_atter_trainer.train(epoch)

            # self.i2a_atter.load_state_dict(
            #     torch.load(os.path.join(base, f'i2a_atter/i2a_att_epoch:{epoch}_best.pt'), map_location=self.device))
...
class I2A_Clser_Trainer:
    ...
    def _train_subepoch(self, epoch, subepoch):
        ...
        if self.do_valation:
            ...
            if self.whole_trainer.do_test == 1:
    ...
    def _test_subepoch(self, epoch, subepoch):
        ...
        # self.mask_gt[ind],self.att_gt[ind],self.ccce_gt[ind],self.z[ind],self.y[ind]
        for batch_idx, (_, att_gt, _, z, _) in enumerate(self.whole_trainer.test_data_loader):
            test_mask = self.whole_trainer.public_tensors['test_i2a_attention_mask'][batch_idx].to(
                self.whole_trainer.device)
            test_z = z.to(self.whole_trainer.device)
            test_att = att_gt.long().to(self.whole_trainer.device)
            with torch.no_grad():
                preds = self.i2a_clser(test_mask * test_z)
                celoss = self.i2a_clser_loss(preds, test_att)
            global_step = (epoch * self.i2a_clser_subepochs + subepoch) * len(self.whole_trainer.test_data_loader) + batch_idx
            self.whole_trainer.writer.set_step(global_step,'test')
            self.test_losses.update('i2a_celoss', celoss.item())
            idx = 0
            for met in self.i2a_clser_metrics:
                self.test_metrics.update(self.test_metrics.keys[idx], met(preds, test_att))
                idx += 1
        ...
        for subepoch in tqdm(range(0, self.i2a_clser_subepochs),
            ...
            if best:
                ...
                with torch.no_grad():  # use best clser for now,update public_tensors['train_i2a_att_feat'] public_tensors['val_i2a_att_feat']
                    self.whole_trainer.public_tensors['test_i2a_att_feat'] = []
                    self.whole_trainer.public_tensors['val_i2a_att_feat'] = []
                    self.whole_trainer.public_tensors['train_i2a_att_feat'] = []

                    self.whole_trainer.public_tensors['test_i2a_att_pred'] = []
                    ...
                    if self.whole_trainer.do_test == 1:
                        for batch_idx, (_, _, _, z, _) in enumerate(self.whole_trainer.test_data_loader):
                            ...
                            test_z = z.to(self.whole_trainer.device)
                            with torch.no_grad():
                                pred_feat = self.i2a_clser.forward_feature(
                                    test_mask * test_z)  # by ljw 20240625,pred_feat (B,6,C)
                                self.whole_trainer.public_tensors['test_i2a_att_feat'].append(
                                    pred_feat)  # by ljw 20240625,pred change to feature format

                                preds = self.i2a_clser(test_mask * test_z)  # todo by ljw 20240702
                                i2a_att_pred = att_preds_totensor(preds).detach().clone()  # todo by ljw 20240702
                                self.whole_trainer.public_tensors['test_i2a_att_pred'].append(
                                    i2a_att_pred)  # todo by ljw 20240702
...
class A2L_Clser_Trainer:
    ...
    def _train_subepoch(self, epoch, subepoch):
        ...
        for batch_idx, (_, _, ccce_gt, _, y) in enumerate(self.whole_trainer.train_data_loader):
            train_ccce = ccce_gt.reshape(y.size(0),6,-1).float().to(
            ...
            preds = self.a2l_clser((train_att * train_ccce).reshape(train_y.size(0), -1))# by ljw 20240625 (B,6,C)*(B,6,C)->(B,6*C)
    ...
    def _val_subepoch(self, epoch, subepoch):
        ...
        for batch_idx, (_, _,ccce_gt, _, y) in enumerate(self.whole_trainer.val_data_loader):
            val_ccce = ccce_gt.reshape(y.size(0), 6,-1).float().to(self.whole_trainer.device)  # by ljw 20240628 (B,6)->(B,6,1) or (B,96)->(B,6,16)
    ...
    def _test_subepoch(self, epoch, subepoch):
        ...
        for batch_idx, (_, _, _, _, y) in enumerate(self.whole_trainer.test_data_loader):
            test_ccce = self.whole_trainer.public_tensors['test_a2l_attention_mask'][batch_idx].\
                reshape(y.size(0), 6,-1).float().to(self.whole_trainer.device)  # by ljw 20240628 (B,6)->(B,6,1) or (B,96)->(B,6,16)

            test_att = self.whole_trainer.public_tensors['test_i2a_att_feat'][batch_idx].float().to(self.whole_trainer.device)#(B,6,C)
            test_y = y.to(self.whole_trainer.device)

            with torch.no_grad():
                preds = self.a2l_clser((test_att * test_ccce).reshape(test_y.size(0),-1))  # by ljw 20240625 (B,6,C)*(B,6)->(B,6*C)
                celoss = self.a2l_clser_loss(preds, test_y)

            global_step = (epoch * self.a2l_clser_subepochs + subepoch) * len(
                self.whole_trainer.test_data_loader) + batch_idx
            self.whole_trainer.writer.set_step(global_step,'test')

            self.test_losses.update('a2l_celoss', celoss.item())
            idx=0
            for met in self.a2l_clser_metrics:
                self.test_metrics.update(self.test_metrics.keys[idx], met(preds, test_y.int()))
                idx+=1
        log_metrics = self.test_metrics.result()
        log_losses = self.test_losses.result()
        return {**log_metrics, **log_losses}  # merge 2 dicts
...
class A2L_Atter_Trainer:
    def __init__(self, a2l_atter, a2l_imp, optim_a2l_atter, a2l_atter_metrics, a2l_atter_losses, a2l_atter_loss,
        ...
        self.tmp_modified_att = {'train_modified_att': [], 'val_modified_att':[],'test_modified_att':[]}
    ...
    def get_pred_match_ccce(self):#todo by ljw 20240701
        ...
        self.whole_trainer.public_tensors['test_pred_ccce_gt'] = []
        tmp=self.whole_trainer.config["data_loader"]["args"]["ccce_file"]
        topk=int(str(self.whole_trainer.config["data_loader"]["args"]["ccce_file"])[-5])

        with torch.no_grad():
            for batch_idx, (_, _, _, _, y) in enumerate(self.whole_trainer.test_data_loader):
                test_y=y.to(self.whole_trainer.device)
                pred_att = self.whole_trainer.public_tensors['test_i2a_att_pred'][batch_idx].to(
                    self.whole_trainer.device)
                pred_ccce_gt=pred_to_ccce(pred_att, test_y,
                             ['subtlety','calcification','margin','spiculation','lobulation','texture'],
                             'malignancy', topk).to(self.whole_trainer.device)

                self.whole_trainer.public_tensors['test_pred_ccce_gt'].append(pred_ccce_gt)
                # print("test_pred_ccce_gt:")
                # print(pred_ccce_gt)
    ...
    def _test_subepoch(self, epoch, subepoch):
        ...
        self.test_metrics.reset()
        self.test_losses.reset()
        self.tmp_modified_att['test_modified_att'] = []

        for batch_idx, (_, _, ccce_gt, _, _) in enumerate(self.whole_trainer.test_data_loader):
            # test_ccce = ccce_gt.to(self.whole_trainer.device)
            test_ccce = self.whole_trainer.public_tensors['test_pred_ccce_gt'][batch_idx]  # todo by ljw 20240702

            test_att = self.whole_trainer.public_tensors['test_i2a_att_feat'][batch_idx].to(self.whole_trainer.device)
            test_att = test_att.reshape(test_att.size(0), -1)  # by ljw 20240625 (B,6,C)->(B,6*C)
            with torch.no_grad():
                self.a2l_imp.reset(test_att)
                inner_log = self.a2l_imp.solve()
                diff = self.a2l_imp.att - test_att  # diff between att* and  att_hat

                diff_reshape = diff.reshape(test_ccce.size(0), 6, -1).to(self.whole_trainer.device)  # by ljw 20240627 diff (B,6*C)->(B,6,C)
                test_ccce_expanded = test_ccce.unsqueeze(-1).repeat(1, 1, diff_reshape.size(
                ...
                self.tmp_modified_att['test_modified_att'].append(modified_att)
                if self.thresh_hold<1:
                    a2l_attention_mask = digitize(
                    self.a2l_atter(test_att.to(self.whole_trainer.device).float().detach().clone()),self.thresh_hold)#todo by ljw 20240702
                else:
                    a2l_attention_mask = get_topk(
                        self.a2l_atter(test_att.to(self.whole_trainer.device).float().detach().clone()),C=16,topk=self.thresh_hold)  # todo by ljw 20240702

                # print("test a2l ccce pred:")
                # print(a2l_attention_mask[:,::16])
                ...
                idx=0
                for met in self.a2l_atter_losses:
                    self.test_losses.update(self.test_losses.keys[idx], met(diff_reshape,test_ccce_expanded).item())
                    idx+=1
                idx=0
                for met in self.a2l_atter_metrics:
                    self.test_metrics.update(self.test_metrics.keys[idx], met(a2l_attention_mask,test_ccce_expanded.reshape(test_ccce_expanded.size(0),-1)))
                    idx+=1
        ...
        for subepoch in tqdm(range(0, self.a2l_atter_subepochs),
            ...
            if best:
                ...
                with torch.no_grad():  # use this epoch best atter,update public_tensors['train_a2l_attention_mask'] public_tensors['train_modified_att']
                    ...
                    self.whole_trainer.public_tensors['test_a2l_attention_mask'] = []
                    self.whole_trainer.public_tensors['test_modified_att'] = []
                    ...
                    if self.whole_trainer.do_test == 1:
                        for batch_idx, (_, _, _, _, _) in enumerate(self.whole_trainer.test_data_loader):
                            test_att = self.whole_trainer.public_tensors['test_i2a_att_feat'][batch_idx].to(
                                self.whole_trainer.device)
                            test_att = test_att.reshape(test_att.size(0), -1)  # by ljw 20240625 (B,6,C)->(B,6*C)
                            if self.thresh_hold < 1:
                                a2l_attention_mask = digitize(
                                    self.a2l_atter(test_att.to(self.whole_trainer.device)).float().detach().clone(),
                                    self.thresh_hold)  # todo by ljw 20240702
                            else:
                                a2l_attention_mask = get_topk(
                                    self.a2l_atter(test_att.to(self.whole_trainer.device)).float().detach().clone(),
                                    C=16,topk=self.thresh_hold)  # todo by ljw 20240704

                            self.whole_trainer.public_tensors['test_a2l_attention_mask'].append(a2l_attention_mask)
                        self.whole_trainer.public_tensors['test_modified_att'] = deepcopy(
                            self.tmp_modified_att[
                                'test_modified_att'])  # use tmp_modified_att,avoid call i2a_imp here will affect atter parameters
...
class I2A_Atter_Trainer:
    ...
    def _train_subepoch(self, epoch, subepoch):
        ...
        for batch_idx, (mask_gt, _, _, z, _) in enumerate(self.whole_trainer.train_data_loader):
            ...
            alignloss = self.i2a_atter_loss(diff, train_mask) * (
                torch.tensor(self.i2a_att_mult).to(self.whole_trainer.device))  # todo by ljw 20240731
    ...
    def _test_subepoch(self, epoch, subepoch):
        ...
        for batch_idx, (mask_gt, _, _, z, _) in enumerate(self.whole_trainer.test_data_loader):
            ...
            modified_att = self.whole_trainer.public_tensors['test_modified_att'][batch_idx].to(
                self.whole_trainer.device)

            with torch.no_grad():
                self.i2a_imp.reset(test_z, modified_att)
                inner_log = self.i2a_imp.solve()  # imp inner loop
                diff = self.i2a_imp.z - test_z  # difference between z* and z

                i2a_attention_mask = digitize(
                    self.i2a_atter(test_z.to(self.whole_trainer.device)).float().detach().clone())
                ...
                idx=0
                for met in self.i2a_atter_metrics:
                    self.test_metrics.update(self.test_metrics.keys[idx], met(i2a_attention_mask, test_mask))
                    idx+=1
        ...
        for subepoch in tqdm(range(0, self.i2a_atter_subepochs),
            ...
            if best:
                ...
                with torch.no_grad():  # use this epoch best atter,update public_tensors['train_i2a_attention_mask'] public_tensors['train_modified_att']
                    ...
                    self.whole_trainer.public_tensors['test_i2a_attention_mask'] = []
                    if self.whole_trainer.do_test == 1:
                        for batch_idx, (_, _, _, z, _) in enumerate(self.whole_trainer.test_data_loader):
                            test_z = z.to(self.whole_trainer.device)
                            i2a_attention_mask = digitize(
                                self.i2a_atter(test_z.to(self.whole_trainer.device)).float().detach().clone())
                            self.whole_trainer.public_tensors['test_i2a_attention_mask'].append(i2a_attention_mask)
