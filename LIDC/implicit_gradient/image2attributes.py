...
from Counter_align.LIDC.loss.loss import weighted_multitasks_celoss
def get_implicit_i2amodel(solver):
    '''
    ...
    class ImplicitModel(torchopt.nn.ImplicitMetaGradientModule,
        ...
        def reset(self, z, modified_att):
            self.z0 = z
            self.z = nn.Parameter(z.clone().detach_(), requires_grad=True)#optimize object:z
            self.modified_att =modified_att  #self.modified_att
        ...
        def objective(self, ):
            ...
            celoss = weighted_multitasks_celoss(preds, self.target,self.modified_att)#use weighted_multitasks_celoss only calculate  modified_att celoss
        ...
        def solve(self, ):
            ...
            self.target = torch.cat([1 - (mscore >= 0.5).long().clone().detach() for mscore in mscores], dim=-1)#result opposite to z0: y*
