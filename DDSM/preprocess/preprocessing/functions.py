...
from breast_segment_master.breast_segment.breast_segment import breast_segment
import sys
sys.path.append('/home/lijingwen/Projects/Counter_align/baseline/breast_cancer_diagnosis-master/src/breast_segment_master')
@detect_func_err
...
def remove_artifacts(img: np.ndarray, mask: np.ndarray = None, crop: bool = True, **kwargs) \
    ...
    breast_mask, bbox = breast_segment(img, threshold=255, threshold_min=30)
...
@detect_func_err
def remove_artifacts_crop(img: np.ndarray, mask: np.ndarray = None, crop: bool = True, **kwargs) \
        -> Tuple[Any, np.ndarray, Any, Any]:
    """
    Función con el pipeline utilizado para remover los artefactos de una imágen mamográfica y obtener exclusivamente
    la zona del seno
